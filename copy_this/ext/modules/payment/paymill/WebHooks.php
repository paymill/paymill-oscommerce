...
class WebHooks extends WebHooksAbstract
{
    ...
    function getEventList(){
        $eventList = array(
            tep_href_link('../WebhookListener.php', '&action=chargeback&type='.$this->_request['type'], 'SSL', false, false) => 'chargeback.executed',
            tep_href_link('../WebhookListener.php', '&action=refund&type='.$this->_request['type'], 'SSL', false, false) => 'refund.succeeded'
        );
    ...
    function loadAllWebHooks($type)
    {
        ...
        $result = array();
        while($row = tep_db_fetch_array($store)){
            $result[] = $row['id'];
        }
    ...
    function updateOrderStatus()
    {
        ...
        try{
            ...
            if (isset($orderStatus) && isset($orderId)) {
                tep_db_query("UPDATE " . TABLE_ORDERS . " SET orders_status='" . $orderStatus . "' WHERE orders_id='" . $orderId . "'");
            }
    ...
    {
        try{
            $statusArray = tep_db_fetch_array(tep_db_query("select orders_status_id from " . TABLE_ORDERS_STATUS . " where orders_status_name = 'Paymill [$statusName]' limit 1"));
